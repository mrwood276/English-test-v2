...
| `list_exam_activity(p_include_templates boolean)` | Every exam that has sessions, with `{sessions, finished, in_progress, pending_essays, average, last_submitted_at, essay_questions}`. Feeds the Grading hub and the Results hub. |
| `list_exam_results(p_exam_id)` | The results table of one exam: `exam`, `summary` (`with_result, in_progress, average, highest, lowest, passed, failed, not_final, pending_essays`) and `rows` (name, class, `class_display` through `class_aliases` — BR-15, score, points, right/wrong, time used, remaining time, page leaves, session status, result status, pending essays, retake state). |
