...
The database migrations live only in the Supabase project (ISSUE-001), so this file is the
repository-side source for the exam SQL functions. The owner runs the block once in
Dashboard → SQL Editor (or saves it as migration `v2_13_exams` with `supabase db pull` afterwards —
either way the live project remains the source of truth).

Tables `exams`, `exam_questions`, `exam_sessions`, `retake_permissions` already exist (from the
Phase-1 migrations); this file only adds the business-rule functions, per DEC-004 (rules in SQL,
one transaction per action, audit inside the transaction). Columns follow `docs/design.md`:

- `exams`: `id, title, description, status (draft/open/closed), duration_minutes, passing_grade,
  availability_mode (manual/scheduled), starts_at, ends_at, late_start_policy
  (full_duration/cut_at_end), access_code, selection_mode (manual/auto), auto_filter jsonb,
  pool_size, draw_per_student, randomize_questions, randomize_options, result_visibility
  (none/score/score_and_review), essay_pending_display (hide_score/show_partial),
  tab_switch_warn_limit, tab_switch_flag_limit, tab_switch_autosubmit_limit, is_template,
  created_by, created_at, updated_at`
- `exam_questions`: `exam_id, question_id, position, weight`
- `audit_logs`: `actor_id, action, entity_type, entity_id, changes`

If a column name here differs from the live table, the live table wins — adjust the SQL, not the contract.
...
returns table (id uuid, title text, status text, duration_minutes int, passing_grade numeric,
...
begin
  ...
  select b.id, b.title, b.status, b.duration_minutes, b.passing_grade,
         b.availability_mode, b.starts_at, b.ends_at, b.late_start_policy,
         b.access_code, b.selection_mode, b.is_template,
...
begin
  ...
  values (v_id, p->>'title', nullif(p->>'description',''), v_status,
      (p->>'duration_minutes')::int, coalesce((p->>'passing_grade')::numeric, 0),
      v_mode, v_starts, v_ends, coalesce(p->>'late_start_policy','full_duration'), v_code, v_sel,
      (p->>'auto_filter')::jsonb, (p->>'pool_size')::int, coalesce((p->>'draw_per_student')::boolean, false),
      coalesce((p->>'randomize_questions')::boolean, false), coalesce((p->>'randomize_options')::boolean, false),
      coalesce(p->>'result_visibility','none'), coalesce(p->>'essay_pending_display','hide_score'),
...
begin
  ...
  update public.exams set status = p_status, updated_at = now() where id = p_id;
  insert into public.audit_logs (actor_id, action, entity_type, entity_id, changes)
...
begin
  ...
  values (v.title || ' (copy)', v.description, 'draft', v.duration_minutes, v.passing_grade, v.availability_mode,
      null, null, v.late_start_policy, lower(substr(md5(random()::text), 1, 6)), v.selection_mode, v.auto_filter,
...
Notes for the owner (checked against the handler contract in `backend/functions/exams/handler.ts`):

- Errors raised with `hint = 'validation'` reach the teacher as friendly 400 messages; anything else is a hidden 500 (DEC-004, `callRpc`).
- `save_exam` returns the exam id; `remove_exam` returns `'deleted'` or `'closed'`; `exam_code_available` returns a boolean; `regenerate_exam_code` returns the new code; `duplicate_exam` returns the new draft id.
- The access-code alphabet is the confusion-safe one from `_shared/codes.ts` (`ABCDEFGHJKLMNPQRSTUVWXYZ23456789`) — keep it identical to what students type.
- This SQL was written from the repository side and **has not been run against the live database yet** (ISSUE-001 keeps the real migrations out of git). When the owner runs it, that live run — not this file — becomes the verified state, and TASK-009's SQL-level verification is done.
