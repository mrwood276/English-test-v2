-- TASK-020 follow-up, committed verbatim with its sibling (DEC-028): the first version of
-- `find_duplicate_groups` set `search_path = public`, so the trigram `%` operator and
-- `similarity()` (installed in the `extensions` schema) would not resolve. This replaces the
-- function with `search_path = public, extensions`. Nothing else changed.
--
-- APPLIED LIVE (2026-09-25 06:06 UTC) as the tracked migration `20260925060638` /
-- `v2_17_duplicate_overview_fix_search_path`, 31 seconds after the first one. Both rows exist in the
-- live migration history, so both files are kept — the sequence is the honest record of what the
-- live database went through. NOT re-applied here; the live function is this version.
--
