...
| F-12 | Essay grading, results, statistics, exports | CORE COMPLETE (Questions statistics and XLSX/PDF exports remain) | ACTIVE | LIVE-VERIFIED (core, 2026-09-24); Classes statistics TESTED |
...
- Not built (TASK-012 remainder): the **Questions** tab (mockup 15 — hardest questions and option distribution) and XLSX/PDF exports. The Classes tab now aggregates merged classes, student counts, averages, passed and not-final counts from the existing overview payload. The results screen also offers a dependency-free CSV download.
