...
| F-12 | Essay grading, results, statistics, exports | CORE COMPLETE (statistics and XLSX/PDF exports remain; CSV export added) | ACTIVE | LIVE-VERIFIED (core, 2026-09-24); CSV export TESTED against mocked results screen |
...
- Not built (TASK-012 remainder): the **Questions** and **Classes** tabs (mockup 15 — hardest questions, option distribution, average per class) and XLSX/PDF exports. The results screen now has a dependency-free CSV export from its existing overview payload; statistics are readable from the stored `exam_results.review_snapshot`, so no schema work is needed.
