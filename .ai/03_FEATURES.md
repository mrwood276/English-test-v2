...
| F-14 | Audit log viewer (built), scheduled purge jobs (built), backups (built), notifications, user management | **PARTIAL — viewer LIVE-VERIFIED (2026-09-25); scheduled jobs LIVE-VERIFIED (2026-09-26); backups LIVE-VERIFIED (2026-09-26)** | ACTIVE | Viewer, the four `pg_cron` jobs and the backup slice are live and verified (`docs/sql-jobs.md`, `docs/sql-backups.md`); notifications/user management not built |
...
- Limitations: the editor cannot reorder files; passage files are edited only in the passage dialog. (`purge_unused` was manual until 2026-09-26: the `purge-orphan-media` cron job now sweeps unused uploads nightly through this action — DEC-029. The admin button still exists for an on-demand sweep; there is still no UI listing unused files.)
...
- Not built: nothing — the exam-wide "add time to everyone" (2026-09-24), the real-browser run (TASK-022, 20/20) and the scheduled `expire_sessions` (2026-09-26, the `expire-sessions` cron job — DEC-029) all exist now.

## F-14 Audit log viewer (built), backups (built), notifications, user management

- Status: **viewer LIVE-VERIFIED (2026-09-25); backups LIVE-VERIFIED (2026-09-26)** — the migrations and both Edge Functions are applied/deployed on the real project, and the rolled-back SQL tests, the admin smoke calls and the live end-to-end checks passed against it. Protection: ACTIVE.
...
- **Scheduled housekeeping jobs (second TASK-015 slice) — LIVE-VERIFIED 2026-09-26 (DEC-029).** `pg_cron` 1.6.4 + `pg_net` 0.20.4 are enabled by `supabase/migrations/20260926002454_scheduled_housekeeping_jobs.sql` and three jobs run as `postgres`: `expire-sessions` every five minutes (BR-21: abandoned attempts become `auto_submitted`/`timed_out` by themselves), and nightly `purge-rate-limits` (spent join/code windows) and `purge-orphan-media` — the last one calls the deployed `media` function over `pg_net` with `{"action": "purge_unused"}` and a **housekeeping key** (generated into Vault, mirrored as the function secret `HOUSEKEEPING_KEY`, SHA-256-compared, opening that one action only), because the file **bytes** can only be removed through the Storage API and a database-only job would delete the rows and leak the files. Schedules are UTC: 19:19 and 19:29 = 02:19/02:29 Jakarta. Files: the migration, `docs/sql-jobs.md`, `supabase/tests/scheduled_jobs_test.sql` (configuration, run live on 2026-09-26: `SCHEDULED JOBS TESTS PASSED (3 jobs active, key 64 chars, nothing written)` — the file now asserts **four** jobs, the nightly backup included), `frontend/tests/live_housekeeping_check.py` (end to end, **40/40**: the jobs fired through `cron.job_run_details`, `net._http_response` 200, the attempt closed, the stale row went, the bytes left the bucket), `_shared/auth.ts#isScheduledJob` + the `media` handler's scheduled door (one action; `media.test.ts` pins the refusals), backend tests 120 → **122**.
- **ISSUE-026 fixed in the same change**: `signed_urls` used to answer a 500 when none of the requested ids exist any more (Storage refuses to sign an empty list); it now answers `{urls: {}}`, with a pinned unit test. Redeployed live.
- **Backups (third TASK-015 slice) — LIVE-VERIFIED 2026-09-26 (DEC-030).** An admin takes a copy by hand on `#/backups` (menu item "Backups", `cloud` icon) and the project keeps taking them on its own: `supabase/migrations/20260926010636_backup_functions.sql` (applied live, tracked as `20260926010636 backup_functions`) + the `backups` Edge Function (deployed) + the fourth job `nightly-backup` (`41 19 * * *` = 02:41 Jakarta, after both purges). **A backup is one ZIP** in the new private `backups` bucket (50 MB cap, signed links only): `data.json` holds every table read through the `public.backup_tables()` allowlist (22 names — the contract), the media manifest and the applied migrations; `media/<path>` holds the bytes of every file really attached to a question or a reading text (the owner's choice: everything including media). The database reads itself and keeps the `backups` table honest (`build_backup_payload`, `record_backup`, `list_backups`, `get_backup`, `delete_backup` — 7 new functions), while zipping, uploading, signing and deleting are the Edge function's job, since only the Storage API can move bytes (the DEC-029 split). Retention: the newest **7 automatic** copies are kept and **manual ones until an admin deletes them** (rows prune in SQL, files go through the function; both audited). The nightly job reuses the DEC-029 housekeeping key and may `create` an automatic copy and **nothing else**; a teacher is refused (403), and a tokenless or wrongly-keyed call is 401. A copy that cannot hold every file says so (`included: false` + `reason`, shown as a warning on the screen) instead of failing silently. Files: the migration, `docs/sql-backups.md`, `supabase/tests/{backup_functions_test.sql,scheduled_jobs_test.sql}`, `backend/functions/backups/{handler,zip,index}.ts`, `backend/tests/backups.test.ts` (8), `frontend/assets/js/teacher/{api/backups.js,screens/backups.js,router.js,screens/shell.js}`, `frontend/tests/{backups_e2e.py,mock_server.py,live_backup_check.py}`. Evidence: backend tests 122 → **130**, `backups_e2e.py` **42 checks** (CI runs it as the twelfth browser suite; menu assertions 7 → 8), SQL tests run live (`SCHEDULED JOBS TESTS PASSED (4 jobs active…)`, `BACKUP TESTS PASSED (7 functions, 22 tables, 0 stored copies…)`), and `live_backup_check.py` **63/63** — a real archive downloaded through its signed link and opened, byte-identical media inside, counts equal to the live ones, the owner's real exam inside, the eighth automatic copy pruning exactly one row **and its file**, the job fired by pg_cron itself (`net._http_response` 200), then everything deleted again.
- Not started (rest of TASK-015): notifications (dashboard + email — needs the owner to choose a provider, DEC-017), user management.
