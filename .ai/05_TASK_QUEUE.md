...
**Next recommended: TASK-013 (the live monitor)** — everything it needs is already recorded (`session_events`, `exam_sessions.tab_switch_count`/`last_heartbeat_at`, `get_session_report`, `list_exam_results` with in-progress rows). Smaller alternative: the TASK-012 remainder (Questions/Classes statistics tabs, mockup 15, and the exports). Small owner steps also still open: disable public sign-up (ISSUE-007), the live media upload check (TASK-007), and `supabase db pull` for the older migrations (TASK-008).
