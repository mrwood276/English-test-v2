...
**Next recommended: the TASK-012 remainder** (the Questions statistics tab, then XLSX/PDF exports; Classes and CSV are implemented and tested) or **TASK-014** (dashboard/UX polish) — both are ordinary feature work with no owner dependency. Release is down to the **three owner-only items** below — see `08_HANDOFF.md` RELEASE STATUS.
...
- Remaining: the **Questions** tab (hardest questions and option distribution) and XLSX/PDF exports. The Classes tab now aggregates `overview.rows`; the results screen also offers a dependency-free CSV download. Question-level statistics still need a staff-facing `review_snapshot` contract.
