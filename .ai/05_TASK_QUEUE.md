...
**TASK-006, step 3: deploy `question-bank` (repository code) so the import actions go live.**
Reason: the entire browser side is finished and tested (parsers, screen, templates, browser tests); the screen is already reachable at `#/questions/import` but the import will fail with "Nothing was saved" until the deployed function has `import_check`/`import`. This step needs the Supabase project (owner, or an agent with Supabase access) — mark it BLOCKED if that access is unavailable. Step 4 (owner review of the formats) follows; see TASK-007 and TASK-008 for parallel work.
Owner-facing steps are written up in `docs/verification-checklist.md` (added 2026-09-22). Read-only live probes that day confirmed: the deployed `question-bank` enforces its auth wall (401 "Please sign in."), anon REST reads are refused (401 `permission denied`), the private bucket leaks nothing, and **public email sign-up is still enabled** (ISSUE-007) — that is the one urgent dashboard flip.
...
- Priority: HIGH. Status: **IN_PROGRESS** (browser side done; deploy step left). Feature: F-08.
