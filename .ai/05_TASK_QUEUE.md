...
**Next recommended: TASK-015** (scheduled `expire_sessions()` / `purge_rate_limits()`, audit-log viewer, notifications — ordinary code work with no owner dependency) or **TASK-020** (duplicate-overview banner; needs a new report action). The **PDF class summary** remains the last piece of TASK-012 but needs the owner's decision on what a one-page class summary should hold. **TASK-022, TASK-008 and TASK-014 are done.** Release is down to the **two owner-only items** below — see `08_HANDOFF.md` RELEASE STATUS.
