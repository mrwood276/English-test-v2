...
**Next recommended: TASK-012 (grading, results, live monitor)** — a result that contains an essay stays `pending_review` until a teacher grades it, and there is no screen for that yet; the same phase brings add time / reopen a session (BR-11), the live monitor, and results/statistics. Small owner steps also still open: disable public sign-up (ISSUE-007), the live media upload check (TASK-007), and `supabase db pull` for the older migrations (TASK-008).
...
- Still open in this phase (moved to TASK-012): **add time / reopen a session (BR-11)** and the remedial UI; `expire_sessions()` is written but not scheduled yet (TASK-015).

## TASK-012 — Phase 4: grading, results, statistics, exports — **NEXT** (mockups 13–15; BR-07, BR-08, BR-11, BR-15, BR-18).
Grading essays is the piece that closes the student loop: a session with a written answer stays `pending_review` (the student sees "waiting for your teacher") until `answer_grades` rows exist and `exam_results` is recalculated. Also here: the results list per exam, merging class-name variants (BR-15), correcting a short answer by hand (BR-18), and **add time / reopen a session (BR-11)** — the SQL already stores `extra_seconds` and accepts the `reopened` status, so no schema work is needed.
