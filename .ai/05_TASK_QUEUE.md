...
- Priority: MEDIUM. Status: **COMPLETE (2026-09-24, Codex — first CI run of the new suite to be watched)**. Feature: F-03.
- Done:
...
- Verification: migration applied live via Supabase MCP and checked (empty activity list with 0 exams, `pg_proc.proacl` shows service-role-only execute, security advisor unchanged); backend **106** ✓, unit **31** ✓, `deno check` on the changed JS ✓, `git diff --check` ✓. New `frontend/tests/dashboard_e2e.py` covers the open card, attention/recent rows, clipboard, confirm/cancel actions, phone menu and no page errors; Python is unavailable on this Windows agent, so the first run is CI. `teacher_e2e.py` covers the empty dashboard.
