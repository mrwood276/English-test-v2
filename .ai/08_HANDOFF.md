...
## Last Agent
Buffy (Freebuff desktop agent, fourth session — third was live verification + checklist; direct git access to `origin`; **read-only Supabase access via the v2 publishable key, no dashboard/CLI**).

## Date
2026-09-22 (fourth session)

## Last Completed Task
- **TASK-009 teacher-side exams built** (fourth session): `exams` Edge Function (8 actions) + 24 Deno tests (backend 41 → 65); exams list + editor screens per mockup 11 (manual/auto selection, schedule, live code check, tab limits 1/3/5, templates, duplicate, leave guard); routes `#/exams*`; Exams menu item now live; mock-server exams handlers; `exams_e2e.py` (25 checks); `teacher_e2e.py` nav assertion updated. **The exam SQL lives in `docs/sql-exams.md` and has NOT been run on the live database** — the screens work only against the mock until it is.
- Third session: **read-only live verification of the v2 project** (publishable key only): anon lockdown confirmed (REST 401 `permission denied`), `question-bank` auth wall confirmed (401 "Please sign in."), private bucket leaks nothing, CORS `*` as expected, and **public email sign-up is still ON** (ISSUE-007 now OPEN, not NEEDS_VERIFICATION); exam tables confirmed present in the live DB.
...
## Database Changes
None applied live this session. **The SQL for TASK-009 is in `docs/sql-exams.md` and has not been executed against the live database** (needs the owner or an agent with SQL access). Live database still at migration `v2_12_import_questions`; migrations still not stored in git (ISSUE-001). The exam tables themselves (`exams`, `exam_questions`, `exam_sessions`, `retake_permissions`) already exist live — verified read-only (401 permission denied, not "does not exist").

## Current State (what works)
- Everything from earlier handoffs still holds: sign-in, question bank, editor, media (built; live upload unverified), import screen (mock-tested) per `03_FEATURES.md`.
- New: exams list `#/exams` and editor `#/exams/new`, `#/exams/edit/:id` — full create/edit flow (manual/auto question selection, schedule, live code uniqueness check, shuffle, tab limits, result rules, templates, duplicate) works against the **mock server** (25 browser checks) with 24 new backend Deno tests. The Exams menu item is now live.
- **Exam saving fails against the real backend until `docs/sql-exams.md` is run and the `exams` function deployed** — the live project has the tables but none of the `save_exam`/`list_exams`/… functions, and no `exams` Edge Function.
- Verification levels: exam UI + backend logic = TESTED (mocked); live exam flow = UNVERIFIED (needs SQL run + deploy); import still as below.
...
**If you have Supabase SQL access on v2**: run `docs/sql-exams.md`, deploy the `exams` function, and live-verify the exam flow (then update `03_FEATURES.md` F-09). Also deploy `question-bank` v3 (TASK-006 step 3) in the same sitting. If you do not: repository-only work left — ISSUE-010 (duplicate banner), ISSUE-011 (reading-text UI), or start TASK-010 (student app) only with the owner's go-ahead since exam live behavior is still unverified.
...
- Do not invent a different exam SQL contract than `docs/sql-exams.md` without reading it first — the Edge Function handler and its tests match that exact function set and argument shape (`p`, `p_id`, `p_actor`, …).
