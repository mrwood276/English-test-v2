...
| Current Development Branch | **`ai-development`** — branched from `main` at `46803f0`; holds the import parsers/screen, the exams work, the CI flake fix (`f7152e5`) and (2026-09-23) the student exam engine. This is where you work. |
...
## Last Agent
Buffy (Freebuff desktop agent, **seventh session** — the student exam engine; direct git access to `origin`; **live Supabase access via the owner's access token (CLI) + the admin account for the flow check — both session-only, never stored in the repo**).
...
- Seventh session: the same access token was used again (CLI only, `SUPABASE_ACCESS_TOKEN`) plus one admin password-grant sign-in for the live student-flow check. Nothing was written into the repo, scripts, or logs beyond the fact that it happened. **The token should be revoked (Dashboard → Access Tokens) and the admin password rotated**, since both have now travelled through chat twice.
...
Zero data changes left behind by either session (all verification rows deleted; live state after the seventh session: 0 exams, 0 sessions, 0 answers, 0 results, 0 events, 0 rate-limit rows, 40 questions, 4 audit rows). Older migrations `v2_01`..`v2_12` are still only in the live project (ISSUE-001 partially fixed; `supabase db pull` brings the rest in).
...
## Remaining Work
1. **TASK-012 (next)**: essay grading — until a teacher can write the `answer_grades` rows and finalize `exam_results`, every exam with a written answer stays `pending_review` (ISSUE-017). Also here: results/statistics screens, **add time / reopen a session (BR-11)** (the SQL already stores `extra_seconds` and accepts `reopened`), and the live monitor.
2. **Owner steps**: disable public sign-up (ISSUE-007), one live media upload check (TASK-007), `supabase db pull` for `v2_01`..`v2_12` (TASK-008), TASK-006 step 4 (review the import formats, one real Excel file), and one owner run of `frontend/index.html` against the real project (the student screens were only mock-tested).
3. **Later**: a scheduled `expire_sessions()` + `purge_rate_limits()` (TASK-015, ISSUE-018/ISSUE-012).

## Known Problems
See `09_KNOWN_ISSUES.md`. Most important now: **ISSUE-017 (no grading screen, so an essay result can never become final)**, ISSUE-007 (public email sign-up still enabled — one dashboard flip), ISSUE-002 (media upload never run live), ISSUE-001 (older migrations still not in git), ISSUE-018 (nothing schedules `expire_sessions`), ISSUE-019 (no `beforeunload` guard on the student page, deliberate).

## Recommended Next Task
**TASK-012 — grading, results and the rest of the exam loop.** It is the next dependency: a student who writes an essay can never get a final mark until the teacher side exists (ISSUE-017). Start with the essay grading screen (mockup 13) plus the small SQL functions it needs (`answer_grades` rows with `is_auto = false`, then recalculate `exam_results` and flip `pending_review` → `graded`), then **add time / reopen a session (BR-11)** — `exam_sessions.extra_seconds` and the `reopened` status already exist, so no schema work is needed — and then the results/statistics screens (mockups 14–15). Smaller parallel items: owner format review for import (TASK-006 step 4), TASK-007 media check, ISSUE-010 (duplicate banner).

## Suggested Work For Next AI
1. `git checkout ai-development && git pull`. Read `.ai/00_AI_RULES.md`, this file, `05_TASK_QUEUE.md` TASK-012, then the relevant source — and `docs/sql-sessions.md` before touching anything student-side.
2. Confirm your environment matches: backend **86** tests, unit 21 (with `--allow-read`), and the **seven** Playwright suites green (teacher, bank, editor, media, import, exams, **student**). `python frontend/dev-server.py 8123` serves both apps: the teacher page at `/teacher/index.html`, the student page at `/index.html`. The student SQL test runs with `npx supabase db query --linked --file supabase/tests/session_functions_test.sql` and must print `SESSION ENGINE TESTS PASSED (all rows rolled back)`.
...
5. CI status: both workflows were **green on `b0c56bd`** (Backend tests + Frontend tests, 2026-09-23) — the browser job now runs seven suites including `exams_e2e` and `student_e2e`. Watch every later push the same way; the browser job is the one that flakes (`playwright install --with-deps chromium` is the slow step).
