...
| Current Development Branch | **`ai-development`** — branched from `main` at `46803f0`; holds the import parsers/screen, the exams work, the CI flake fix (`f7152e5`), the student exam engine (2026-09-23) and (2026-09-24) essay grading + the results screens. This is where you work. |
...
5. CI status: both workflows were **green on `b0c56bd`** (2026-09-23) and the frontend job now runs **eight** suites (the eighth, `results_e2e`, was added on 2026-09-24 — watch the first run of it). Watch every push; the browser job is the one that flakes (`playwright install --with-deps chromium` is the slow step).
