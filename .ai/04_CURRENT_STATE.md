...
| Item | Value |
|---|---|
| Last updated | 2026-09-26 (Buffy — **the TASK-015 scheduled purge jobs are LIVE-VERIFIED**: `pg_cron` + `pg_net` now close abandoned sessions every five minutes and, nightly, drop spent rate-limit windows and delete unused uploads through the media function (DEC-029). **ISSUE-012, ISSUE-018 and ISSUE-010 closed**; **ISSUE-026 found and fixed**. No open release blocker) |
...
| Current focus | Nothing is blocked. Remaining work is optional/owner-driven: the **TASK-015 remainder** (backups; notifications need DEC-017; user management), the **PDF class summary** (owner decision), and the merge of `ai-development` into `main` (owner decision). |
| Branch model | `main` = stable. `ai-development` = shared AI development. |
| Current branch / commit | **`ai-development`** at **`e4820a6`** (`eedf6d4` the scheduled jobs + the ISSUE-026 fix, `e4820a6` its docs) — **both workflows green on `e4820a6`** (Backend + Frontend, 2026-09-26); locally this session: backend **122**, unit **31**, live check **40/40**. The previous head was **`d00a77d`** (`cc2b0d5` the duplicate-banner feature, `d00a77d` its docs), pushed with **both workflows green**; the TASK-007 and exam-delete work is below it (`cdad496`/`5252359`/`b799af1`/`b520f80`). Local: backend **120**, unit **31**, all eleven browser suites green (`question_bank` now **67** checks, `exams` 37); live checks 32/32 (media), 17/17 (exam delete rule) and **15/15 (duplicate banner)**. |
...
- **TASK-015**: two slices are **LIVE-VERIFIED** — the audit-log viewer (2026-09-25) and the **scheduled purge jobs (2026-09-26, DEC-029)**. What remains in the task: backups (manual + scheduled), notifications (owner must choose a provider, DEC-017), user management.
...
**TASK-015 remainder** (backups; notifications — needs the owner's provider decision; user management) is the next code task — the scheduled jobs half is done. Owner-dependent items: the PDF class-summary content (TASK-012), import-format review (TASK-006), and the merge of `ai-development` into `main`.
...
1. **TASK-015 remainder**: backups (manual + scheduled — a nightly job can now reuse DEC-029's pattern), notifications (needs the owner's email-provider decision, DEC-017), user management.
