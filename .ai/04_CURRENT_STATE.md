...
| Item | Value |
|---|---|
| Last updated | 2026-09-26 (Buffy — **the TASK-015 backup slice is LIVE-VERIFIED (DEC-030)**: an admin takes a copy by hand on `#/backups` and a fourth `pg_cron` job takes one nightly at 02:41 Jakarta; a copy is one ZIP in a new private bucket holding every table plus the attached media bytes, the newest seven nightly copies are kept and manual ones until deleted, and the whole slice was proved end to end (63/63) — a real archive downloaded and opened, the owner's exam inside, the eighth nightly copy pruning a row **and its file**. Before it in the same session: the scheduled purge jobs (DEC-029), with **ISSUE-012/018/010 closed** and **ISSUE-026 found and fixed**. No open release blocker) |
...
| Development phase | Phase 5 **FULLY LIVE-VERIFIED**; Phase 6 dashboard/UX polish complete and CI-verified; Phase 7 in progress — the audit viewer, the scheduled purge jobs and **backups (manual + nightly)** are **LIVE-VERIFIED**, while notifications and user management remain; **F-07 media is LIVE-VERIFIED too**. |
| Current focus | Nothing is blocked. Remaining work is optional/owner-driven: the **TASK-015 remainder** (notifications need DEC-017; user management), the **PDF class summary** (owner decision), and the merge of `ai-development` into `main` (owner decision). |
| Branch model | `main` = stable. `ai-development` = shared AI development. |
| Current branch / commit | **`ai-development`** at **`d4502da`** (`eedf6d4` the scheduled jobs + the ISSUE-026 fix, `e4820a6` its docs, `d4502da` hashes/CI) — **both workflows green on `e4820a6` and `d4502da`** (Backend + Frontend, 2026-09-26); the backup slice sits **on top of it** (feature + docs commits; see `08_HANDOFF.md` for the hashes). Locally this session: backend **130**, unit **31**, `backups_e2e.py` **42 checks**, live checks **40/40** (housekeeping) and **63/63** (backups). The previous head was **`d00a77d`** (`cc2b0d5` the duplicate-banner feature, `d00a77d` its docs), pushed with **both workflows green**; the TASK-007 and exam-delete work is below it (`cdad496`/`5252359`/`b799af1`/`b520f80`). Local: backend **120**, unit **31**, all eleven browser suites green (`question_bank` now **67** checks, `exams` 37); live checks 32/32 (media), 17/17 (exam delete rule) and **15/15 (duplicate banner)**. |
...
| Extensions | `pg_stat_statements`, `pg_trgm`, `pgcrypto`, `uuid-ossp`, `supabase_vault`, **`pg_cron` 1.6.4** and **`pg_net` 0.20.4** (the last two enabled 2026-09-26 for the scheduled jobs; both were already in `shared_preload_libraries`) |
| Scheduled jobs (`cron.job`, 2026-09-26) | `expire-sessions` `*/5 * * * *` → `select public.expire_sessions()`; `purge-rate-limits` `19 19 * * *` → `select public.purge_rate_limits()`; `purge-orphan-media` `29 19 * * *` → `net.http_post(…/functions/v1/media, x-housekeeping-key: <vault>, {action: purge_unused})`. Job ids 1/2/3, all active, running as `postgres` in `postgres`. Contract: `docs/sql-jobs.md` |
...
## Live system facts (2026-09-26, twentieth and twenty-first sessions — the scheduled jobs and the backups)

- **The clock is live and has four jobs** (`pg_cron` 1.6.4 + `pg_net` 0.20.4 enabled): ids 1/2/3 are the three housekeeping jobs; id **4** is `nightly-backup` (`41 19 * * *`), added by the backup slice below and calling the deployed `backups` function with the same housekeeping key (`create` only).
- **The backup slice is live**: `supabase/migrations/20260926010636_backup_functions.sql` — the private `backups` bucket (50 MB cap), the 22-table allowlist, `build_backup_payload` / `record_backup` (with the retention sweep) / `list_backups` / `get_backup` / `delete_backup`, and the fourth job. **Applied live and recorded as `20260926010636` / `backup_functions` in `supabase_migrations.schema_migrations`** (statements byte-for-byte the file, 11,535 chars), and the `backups` Edge Function is deployed. Live public SQL functions 62 → **69**; buckets `question-media` + `backups`; `public.backups` has RLS on with **0 policies**. Contract: `docs/sql-backups.md` (DEC-030).
- **The purges are live too**: `supabase/migrations/20260926002454_scheduled_housekeeping_jobs.sql`, **applied live and recorded as `20260926002454` / `scheduled_housekeeping_jobs` in `supabase_migrations.schema_migrations`** — this session applied its own migration through the Management API **and inserted the matching tracking row in the same session**, so git and the live project agree and `supabase db push` will not try to re-apply it (the ISSUE-001 drift does not grow).
- **The housekeeping key**: generated inside the migration into `vault.secrets` as `housekeeping_key` (64 hex chars, sha256 `d042d54734f7…`), mirrored as the **`media` function secret `HOUSEKEEPING_KEY`** through the Management API (the one step a migration cannot do), and never written into the repository — the live check greps the whole working tree for it. Rotation and reachability: `docs/sql-jobs.md`.
- **`media` redeployed** with two changes: the scheduled door (`isScheduledJob` in `_shared/auth.ts`, opening **only** `purge_unused`) and the ISSUE-026 fix (`signed_urls` answers `{urls: {}}` instead of a 500 when none of the ids exist any more).
- **Verified live**: `supabase/tests/scheduled_jobs_test.sql` → `SCHEDULED JOBS TESTS PASSED (4 jobs active, key 64 chars, nothing written)` and `supabase/tests/backup_functions_test.sql` → `BACKUP TESTS PASSED (7 functions, 22 tables, 0 stored copies, nothing written)`; `frontend/tests/live_housekeeping_check.py` → **40/40** — the owner's own exam/attempt were counted before and after and left untouched, and every job's effect was observed (session `timed_out` + stamped, stale rate-limit row gone, media row gone **and the bytes gone from Storage**, `net._http_response` HTTP 200, schedules restored).
- Backend tests **122** (121 + the `signed_urls` pin), unit **31**; the live project also has the owner's own `4KHU2A` exam and its submitted attempt (real data — never clean those up).

## Live system facts (re-verified 2026-09-25, eighteenth session — TASK-007 and ISSUE-023)
...
|---|---|---|---|
| `exams` | full handler + parser + 28 Deno tests; screens list + editor | **v4 live** (DEC-027 adds the admin-only permanent delete); SQL applied; flow live-verified | Exam screens work against the real backend |
...
| **Audit viewer (F-14)** | migration `20260929000000_audit_functions.sql` + `audit` Edge Function + `#/audit` screen, all tested | **APPLIED + DEPLOYED** (live; verified end to end 2026-09-25) | No drift — the screen reads the real audit rows |
| **Scheduled jobs (F-14)** | migration `20260926002454_scheduled_housekeeping_jobs.sql` + the `media` scheduled door + `supabase/tests/scheduled_jobs_test.sql` | **APPLIED + DEPLOYED + FIRING** (2026-09-26: three active `cron.job` rows, `media` redeployed, live check 40/40) | No drift — the file name is the live version number and the tracking row was written by the same session |
...
- Verified live: `list_audit_logs` exists, is `security definer` + `stable`, and its ACL is `postgres` + `service_role` only (PUBLIC/anon/authenticated revoked). Live public SQL functions: **61** — that was the count at that moment; it is **62** now, because another session's TASK-020 work arrived live (ISSUE-024).
...
1. **TASK-015 scheduled purge jobs LIVE-VERIFIED (2026-09-26)** — `pg_cron`/`pg_net` enabled live by migration `20260926002454_scheduled_housekeeping_jobs.sql` (tracking row written by the same session, so no new ISSUE-001 drift): `expire-sessions` every five minutes, and nightly `purge-rate-limits` plus `purge-orphan-media` — the last one through the deployed `media` function with a **housekeeping key** kept in Vault and as a function secret (DEC-029), because the file **bytes** can only be deleted through the Storage API. The `media` handler gained the scheduled door (one action only) and the ISSUE-026 fix; backend tests 120 → **122**; live evidence: `supabase/tests/scheduled_jobs_test.sql` passed and `frontend/tests/live_housekeeping_check.py` **40/40** (the jobs really fired: `cron.job_run_details` succeeded, `net._http_response` 200, the session closed, the file's bytes left the bucket, the check's own rows deleted and counts restored). Contract: `docs/sql-jobs.md`.
2. **Leave-guard defect fixed (2026-09-25, git `7a5b09e`)** — the reason the Frontend job failed twice (`f4eb5e7`, `2f5de2c`, both at `exams_e2e.py` line 141: "FAIL existing exam title loaded" then a 30 s timeout on `.chosen-item`). `guard.js` warned the browser on `if (guard)`, and every editor registers a guard on sight, so an *untouched* editor asked "Leave site?" on reload/close, and the router held the URL on the current screen while it asked (it rewrites the hash back with `history.replaceState`). A reload landing in that window loads the **previous** screen: the test's clean draft editor came back, and that draft was saved "by filter" with no `.chosen-item` to wait for. Fix: `setLeaveGuard(fn, hasUnsavedWork?)` + `hasUnsavedChanges()`, the beforeunload warning and the router's ask/URL hold-back gated on real unsaved work, the three guarded screens passing their existing synchronous dirt predicate. `exams_e2e.py` also loads the target screen *before* reloading and now checks an untouched editor reloads with no leave warning (28 checks). Verified by restoring the old semantics: the suite then dies at that reload (`Page.reload` timeout after 21 checks).
...
- **TASK-015**: three slices are **LIVE-VERIFIED** — the audit-log viewer (2026-09-25), the **scheduled purge jobs (2026-09-26, DEC-029)** and the **backups (2026-09-26, DEC-030: `#/backups` + the nightly job, one ZIP per copy holding every table and the attached media bytes, newest 7 nightly / manual until deleted)**. What remains in the task: notifications (owner must choose a provider, DEC-017) and user management.
- **TASK-009's exam delete rule**: **ISSUE-023 is FIXED (2026-09-25, DEC-027)** — the list reports the attempt count, a teacher gets no Delete button on an exam with attempts, and the admin can permanently delete it behind a dialog that names what is lost. Live-verified 17/17 (`frontend/tests/live_exam_delete_check.py`).

## Pending work (see `05_TASK_QUEUE.md`)

**TASK-015 remainder** (notifications — needs the owner's provider decision; user management) is the next code task — the scheduled jobs **and** the backup halves are done. Owner-dependent items: the PDF class-summary content (TASK-012), import-format review (TASK-006), and the merge of `ai-development` into `main`.
...
1. **TASK-015 remainder**: notifications (needs the owner's email-provider decision, DEC-017) and user management. ~~Backups~~ — **done and live-verified 2026-09-26 (DEC-030)**.
