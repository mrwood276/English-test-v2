...
## ISSUE-023 — Deleting an exam that has attempts closes it instead, and the owner retried five times
- Severity: LOW (behavior is deliberate; the question is whether it is what the owner wants). Status: OPEN — needs one owner decision. Related: TASK-009, `remove_exam`.
- Observed (live `audit_logs`, 2026-09-25 07:17–07:42): the owner created an exam titled "test", took it himself (one submitted session by `jonathan`, `XII TKJ A`), then hit Delete five times. `remove_exam` closes an exam that already has sessions instead of deleting it and writes `exam.close` with `reason: "delete requested while sessions exist"` — five such rows. The screen does explain it (`toast("This exam already has sessions, so it was closed instead of deleted.")` in `exams.js`), so nothing is broken or silent; the retries suggest the owner wanted the exam *gone*.
- Why it is deliberate: BR-10 / DEC-012 — results and attempt history never disappear, so an exam with attempts must not vanish. Deleting would orphan `exam_sessions` and its snapshot.
- What is needed: an owner decision. Either keep it as is (and make the message harder to miss, e.g. disable Delete when `session_count > 0` and label it "Close exam"), or add an admin-only hard delete that asks again and removes the sessions too. Do not change it without that decision.
- Live state left behind: one closed exam `3ad8eb38` ("test", code `4KHU2A`) and its one submitted session are still in the live project from that owner test — **that is real owner data, not test junk; do not delete it.**
